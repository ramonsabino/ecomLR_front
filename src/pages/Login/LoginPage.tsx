...
import { Container, Typography, TextField, Button, Box, Card, CardContent, CardActions, Link, CircularProgress } from '@mui/material';
...
const LoginPage: React.FC = () => {
  ...
  const { authToken, login } = useAuth(); // Usando o contexto de autenticação
    ...
    <Container maxWidth="sm" style={{ marginTop: '50px' }}>
      <Card>
        ...
        {!authToken && (
          <>
            <CardActions>
              <Button type="submit" variant="contained" color="primary" fullWidth onClick={isLogin ? handleLogin : handleRegister} disabled={isLoading}>
                {isLoading ? <CircularProgress size={24} /> : (isLogin ? 'Entrar' : 'Registrar')}
              </Button>
            </CardActions>
            <Box textAlign="center" mt={2} mb={2}>
              <Link component="button" variant="body2" onClick={() => setIsLogin(!isLogin)}>
                {isLogin ? 'Não tem cadastro? Se registre' : 'Já tem uma conta? Entre'}
              </Link>
            </Box>
          </>
        )}
